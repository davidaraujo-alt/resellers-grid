...
<div class="pane" id="pane-chart-tpv">
  <div class="chart-wrap">
    <div class="chart-title">TPV M0 e M1 Diario por Nivel</div>
    <div class="chart-sub">Acumulado diario Jan-Abr/26. M1 disponivel a partir de Fev/26. Linha tracejada = M1.</div>
    <div class="chart-controls" id="nivel-filter-tpv"></div>
...
let chartTpv=null,activeNivelTpv='Todos';
function renderChartTpv(){
  const niv=activeNivelTpv==='Todos'?NIV_ORDER:[activeNivelTpv];
  const dates=[...new Set(DAILY.map(d=>d.data))].sort();
  const datasets=[];
  niv.forEach(n=>{
    const map=Object.fromEntries(DAILY.filter(d=>d.nivel===n).map(d=>[d.data,d]));
    datasets.push({label:n+' M0',data:dates.map(d=>map[d]?.tpv_m0||0),borderColor:NIV_COLOR[n],backgroundColor:NIV_COLOR[n]+'22',borderWidth:2,pointRadius:0,fill:false,tension:.3});
    datasets.push({label:n+' M1',data:dates.map(d=>map[d]?.tpv_m1||0),borderColor:NIV_COLOR[n],borderDash:[5,3],borderWidth:1.5,pointRadius:0,fill:false,tension:.3});
  });
  if(chartTpv)chartTpv.destroy();
  chartTpv=new Chart(document.getElementById('chartTpv').getContext('2d'),{
    type:'line',data:{labels:dates,datasets},
    options:{responsive:true,plugins:{legend:{position:'top',labels:{font:{size:11},usePointStyle:true}},tooltip:{mode:'index',intersect:false,callbacks:{label:i=>`${i.dataset.label}: R$ ${(i.raw/1000).toFixed(1)}K`}}},
    scales:{x:{ticks:{maxRotation:45,font:{size:10},maxTicksLimit:20},grid:{display:false}},y:{ticks:{callback:v=>'R$ '+(v/1000).toFixed(0)+'K',font:{size:10}},grid:{color:'#f0f0f0'}}},interaction:{mode:'index',intersect:false}}
  });
  document.getElementById('nivel-filter-tpv').innerHTML=['Todos',...NIV_ORDER].map(n=>`<button class="filter-btn${activeNivelTpv===n?' active':''}" onclick="activeNivelTpv='${n}';renderChartTpv()">${n}</button>`).join('');
}
